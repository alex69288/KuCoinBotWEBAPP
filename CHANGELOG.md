...
## [v0.0.33] - 2025-11-16
- Убран скролл у корневых элементов страницы: `html`, `body`, `#root`. Теперь прокрутка обрабатывается только внутренними контейнерами (например, слайдами и блоками). Изменён `frontend/src/index.css`.

## [v0.0.34] - 2025-11-16
- Обновлено поведение высоты страниц: слайды и контейнеры теперь используют `min-height: calc(100vh - var(--tabs-height))` и не ограничивают `max-height`. Это позволяет:
	- при коротком контенте — занять всю доступную высоту экрана (нет лишнего скролла);
	- при длинном контенте — расширяться по содержимому (страница растёт до высоты контента).
	Изменения в `frontend/src/index.css`.
