...
## Переменные окружения и приоритет .env

Чтобы избежать конфликтов с несколькими `.env` файлами (например, `.env` в корне и `backend/.env`), backend сервер загружает переменные в следующем порядке:

- если найден `backend/.env`, он будет использован (предпочтение для backend-специфичных переменных);
- иначе используется корневой `.env`.

Рекомендации:
- Для локальной разработки рекомендуется хранить все backend-переменные в `backend/.env` и frontend-переменные в `frontend/.env`.
- Если вы хотите единый источник истины для всего проекта, храните один `.env` в корне и удалите `backend/.env`.
- Не храните реальные ключи в репозитории — используйте Amvera Secrets для production.
