 ...
 */
export function calculateTotalProfit(trades, initialBalance) {
 ...
 */
export function calculateBestAndWorstTrades(trades) {
    if (trades.length === 0)
 ...
 */
export function calculateAverageProfitAndLoss(trades) {
...
export function calculateStreaks(trades) {
    ...
    trades.forEach(trade => {
        ...
        if (currentWinStreak > winningStreak)
            winningStreak = currentWinStreak;
        if (currentLossStreak > losingStreak)
