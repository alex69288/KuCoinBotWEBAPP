...
- **Внутренний порт (containerPort)**: 3000 - порт, на котором приложение слушает внутри контейнера
- **Внешний порт (port)**: 443 - порт, на котором Amvera проксирует трафик (HTTPS)
- **PORT в переменных окружения**: 3000 - должен совпадать с containerPort
...
- `PORT` - **ОБЯЗАТЕЛЬНО!** установите на `3000` (внутренний порт, containerPort; внешний порт 443 настраивается в amvera.yaml)
